...
echo "Can I restore your dotfiles now? (y/n)"
read restore_dotfiles
if echo "$restore_dotfiles" | grep -iq "^y" ;then
	cd ~/dotfiles
	stow bash
	stow vim
	stow zsh 
...
echo "For example, mine will be \"Wallace Benevides\""
...
echo "Would you like install nvidia-drive? (y/n)"
read install_drive
if echo "$install_drive" | grep -iq "^y" ;then
...
echo "Would you like restarting now? (y/n)"
read restart
if echo "$restart" | grep -iq "^y" ;then
